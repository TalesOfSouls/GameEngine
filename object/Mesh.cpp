 ...
 */
#pragma once
#ifndef COMS_OBJECT_MESH_C
#define COMS_OBJECT_MESH_C
...
void mesh_from_file_txt(
    ...
    f32* vertices = (f32 *) memory_get(ring, 500000 * sizeof(f32));

    int32 normal_count = 0;
    f32* normals = (f32 *) memory_get(ring, 500000 * sizeof(f32));

    int32 tex_coord_count = 0;
    f32* tex_coords = (f32 *) memory_get(ring, 500000 * sizeof(f32));

    //int32 color_count = 0;
    // f32* colors = (f32 *) memory_get(ring, 500000 * sizeof(f32));

    int32 face_type = VERTEX_TYPE_POSITION;
    int32 face_count = 0;
    int32* faces = (int32 *) memory_get(ring, 500000 * sizeof(int32));
    ...
    while (*pos != '\0') {
        ...
        while (*pos == ' ' || is_eol(pos)) {
            const int32 eol_length = is_eol(pos);
    ...
    if (face_count > 0) {
        ...
        const int32 normal_offset = ((mesh->vertex_type & VERTEX_TYPE_TEXTURE_COORD) ? 1 : 0);
        const int32 texture_offset = ((mesh->vertex_type & VERTEX_TYPE_NORMAL) ? 1 : 0);
...
{
    if (!data) {
        LOG_1("[WARNING] No mesh data provided to load");
        return 0;
    }

    LOG_3("[INFO] Load mesh");

...
struct ChunkMemory {
    ...
    alignas(sizeof(size_t)) uint_max* free;
};
...
FORCE_INLINE
uint_max chunk_size_total(uint32 count, int32 element_size, int32 alignment = sizeof(size_t)) NO_EXCEPT
{
    ...
    return count * element_size
        + sizeof(uint_max) * ceil_div(count, 64U) // free
...
void chunk_alloc(ChunkMemory* const buf, uint32 count, int32 element_size, int32 alignment = sizeof(size_t)) NO_EXCEPT
{
    ...
    element_size = chunk_size_element(element_size, alignment);
    const uint_max size = chunk_size_total(count, element_size, alignment);
    ...
    buf->free = (uint_max *) align_up(
        (uint_max) ((uintptr_t) (buf->memory + count * element_size)),
        (uint_max) alignment
    );
    memset(buf->free, 0, (count + (sizeof(uint_max) * 8 - 1)) / (sizeof(uint_max) * 8));
}
...
) NO_EXCEPT
{
    ...
    buf->last_pos = -1;
    buf->alignment = alignment;
    buf->free = (uint_max *) align_up((uintptr_t) (buf->memory + count * element_size), (sizeof(uint_max) * 8));
    memset(buf->free, 0, (count + (sizeof(uint_max) * 8 - 1)) / (sizeof(uint_max) * 8));
...
{
    return (uint_max *) align_up((uintptr_t) (buf->memory + buf->count * buf->chunk_size), (uint_max) buf->alignment);
}
...
bool chunk_is_free(const ChunkMemory* const buf, uint32 element) NO_EXCEPT
{
    ...
    return !IS_BIT_SET_R2L(buf->free[free_index], bit_index);
}
...
int32 chunk_reserve_one(ChunkMemory* const buf) NO_EXCEPT
{
    ...
    uint32 free_index = (buf->last_pos + 1) / (sizeof(uint_max) * 8);
    uint32 bit_index = MODULO_2(buf->last_pos + 1, (sizeof(uint_max) * 8));

    // Check standard simple solution
    if (!IS_BIT_SET_R2L(buf->free[free_index], bit_index)) {
    ...
    const uint32 total_words = (buf->count + (sizeof(uint_max) * 8 - 1)) / (sizeof(uint_max) * 8);

    for (uint32 w = 0; w < total_words; ++w) {
        if (buf->free[free_index] != OMS_UINT_MAX) {
        ...
        ++free_index;
        if (free_index * (sizeof(uint_max) * 8) >= buf->count) { UNLIKELY
            free_index = 0;
        }
    }
...
int32 chunk_reserve(ChunkMemory* const buf, uint32 elements = 1) NO_EXCEPT
{
    ...
    // Check standard simple solution
    if (elements == 1 && !IS_BIT_SET_R2L(buf->free[free_index], bit_index)) {
        buf->free[free_index] |= (OMS_UINT_ONE << bit_index);
        ++buf->last_pos;
    ...
    while (i++ <= buf->count) {
        if (buf->free[free_index] == OMS_UINT_MAX) {
            ...
            continue;
        } else if (free_index * (sizeof(uint_max) * 8) + bit_index + elements - consecutive_free_bits > buf->count) { UNLIKELY
            // Go to beginning after overflow
            i += buf->count - (free_index * (sizeof(uint_max) * 8) + bit_index);
            consecutive_free_bits = 0;
            free_index = 0;
            bit_index = 0;
        ...
        } while (!IS_BIT_SET_R2L(buf->free[free_index], bit_index)
            && consecutive_free_bits != elements
            && free_index * (sizeof(uint_max) * 8) + bit_index + elements - consecutive_free_bits <= buf->count
            && i <= buf->count
        );
        ...
        if (consecutive_free_bits == elements) {
            ...
            // Mark as used
            if (elements == 1) {
                buf->free[possible_free_index] |= (OMS_UINT_ONE << possible_bit_index);
            } else {
...
{
    buf->free[free_index] &= ~(OMS_UINT_ONE << bit_index);
    DEBUG_MEMORY_DELETE((uintptr_t) (buf->memory + (free_index * (sizeof(uint_max) * 8) + bit_index) * buf->chunk_size), buf->chunk_size);
}
...
HOT_CODE
void chunk_free_elements(ChunkMemory* const buf, uint_max element, uint32 element_count = 1) NO_EXCEPT
{
...
int64 chunk_dump(const ChunkMemory* const buf, byte* data) NO_EXCEPT
{
    ...
    data += buf->size;
        ...
        for (uint32 i = 0; i < (buf->count + (sizeof(uint_max) * 8 - 1)) / (sizeof(uint_max) * 8); ++i) {

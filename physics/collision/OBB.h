...
void compute_aabb_x_primitives(const OBB* obbs, int32 n, AABB1D* arr) {
    for (int32 i = 0; i < n; ++i) {
        ...
        float ex = OMS_ABS_F32(o->axis[0].x) * o->half[0]
            + OMS_ABS_F32(o->axis[1].x) * o->half[1]
            + OMS_ABS_F32(o->axis[2].x) * o->half[2];
...
bool obb_overlap(const OBB *A, const OBB *B)
{
    ...
    for(int32 i = 0; i < 3; i++) {
        for(int32 j = 0; j < 3; j++) {
            ...
            AbsR[i][j] = OMS_ABS_F32(R[i][j]) + 1e-6f;
        }
    }

 ...
 */
#ifndef COMS_STDLIB_HASH_MAP_C
#define COMS_STDLIB_HASH_MAP_C
...
void hashmap_alloc(HashMap* const hm, int32 count, int32 max_count, int32 element_size, int32 alignment = 32) NO_EXCEPT
{
    ...
    max_count = align_up(max_count, 2);
    ...
    chunk_alloc(&hm->buf, count, max_count, element_size, alignment);
}
...
void hashmap_alloc(HashMap* const hm, MemoryArena* mem, int32 count, int32 max_count, int32 element_size, int32 alignment = 32) NO_EXCEPT
{
    ...
    LOG_1("[INFO] Allocate HashMap for %n elements with %n B per element", {DATA_TYPE_INT32, &count}, {DATA_TYPE_INT32, &element_size});
    hm->hash_function = hash_djb2;
    chunk_alloc(&hm->buf, mem, count, max_count, element_size, alignment);
}
...
FORCE_INLINE
void hashmap_free(HashMap* const hm, MemoryArena* mem) NO_EXCEPT
{

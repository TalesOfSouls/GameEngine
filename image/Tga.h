...
void generate_default_tga_references(const FileBody* file, Tga* tga) NO_EXCEPT
{
    ...
    tga->header.horizontal_ordering = file->content[17] & (1 << 4); // 0 = left-to-right
...
void image_tga_generate(const FileBody* src_data, Image* image) NO_EXCEPT
{
    ...
    if ((image->image_settings & IMAGE_SETTING_BOTTOM_TO_TOP) == src.header.vertical_ordering
        && src.header.horizontal_ordering == 0
    ) {
    ...
    const uint32 pixel_rgb_bytes = pixel_bytes - alpha_offset;
    const uint32 width_pixel_bytes = src.header.width * pixel_bytes;

    for (uint32 y = 0; y < src.header.height; ++y) {
        const uint32 row_pos1 = y * image->width * pixel_bytes;
        const uint32 row_pos2 = (image->image_settings & IMAGE_SETTING_BOTTOM_TO_TOP) == src.header.vertical_ordering
            ? y * width_pixel_bytes
            : (src.header.height - y - 1) * image->width * pixel_bytes;

        for (uint32 x = 0; x < src.header.width; ++x) {
            if (src.header.horizontal_ordering == 0) {

...
void dispatcher_alloc(Dispatcher* dispatcher, void* app, int32 category_count, int32 func_count, BufferMemory* const buf, int32 alignment = sizeof(size_t)) {
    ...
    dispatcher->functions = (ControllerFunction **) memory_get(buf, sizeof(ControllerFunction *) * category_count, alignment, true);
    dispatcher->functions[0] = (ControllerFunction *) memory_get(buf, sizeof(ControllerFunction) * func_count, alignment, true);
}
